...
from PatchTST.PatchTST_supervised.models import Informer, Autoformer, Transformer, DLinear, Linear, NLinear, PatchTST
from PatchTST.PatchTST_supervised.utils.tools import EarlyStopping, adjust_learning_rate, visual, test_params_flop
...
class Exp_Main(Exp_Basic):
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, _) in enumerate(train_loader):
                ...
                # decoder input (PatchTST does not use decoder input, only used as arguments for other models, see below.)
                dec_inp = torch.zeros_like(batch_y[:, -self.args.pred_len:, :]).float()
                dec_inp = torch.cat([batch_y[:, :self.args.label_len, :], dec_inp], dim=1).float().to(self.device)
    ...
    def predict(self, setting, load=False):
        ...
        self.model.eval()
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, batch_time) in enumerate(pred_loader):
                batch_x = batch_x.float().to(self.device)
                batch_y = batch_y.float()
                batch_x_mark = batch_x_mark.float().to(self.device)
                batch_y_mark = batch_y_mark.float().to(self.device)
