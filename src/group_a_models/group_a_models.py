...
from Base_Model import BaseModel, DeepLearningModel
...
import json
from helper_funcs import add_lag_features
...
class LSTMModel(DeepLearningModel):
    def __init__(self, config: LSTMBaseConfig, func: Callable[[pd.DataFrame,str ,list[str],list[int] | None, list[int] | None], pd.DataFrame] | None = None):
        ...
        self.model = self.build_model(config, input_size=len(self.config.all_feature_cols)+1)
        
    def build_model(self, config, input_size):
