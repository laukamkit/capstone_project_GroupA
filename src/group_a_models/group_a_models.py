...
class PatchTSTModel(Base_Model):
    ...
    def test_model(self, test_mode=0):
        criterion = self._select_criterion()
        test_data, test_loader = self._get_data(flag='test' if test_mode else 'val')

        if test_mode:
            results_path = os.path.join(self.nsw_data_loader.output_dir, f"{self.config.model}_results")
            if not os.path.exists(results_path):
                os.makedirs(results_path)
            checkpoint_path = os.path.join(self.nsw_data_loader.output_dir, f"{self.config.model}_checkpoints")
    ...
    def train_model(self, task_id):
        ...
        for epoch in range(self.config.training_epochs):
            ...
            _, _, _, _, mae_val, rmse_val, mse_val = self.test_model()
...
class SarimaxModel(Base_Model):
    ...
    def test_model(self, model_fit:SARIMAXResultsWrapper | str | None, test_mode: bool = False):
        ...
        if test_mode:
            ...
            results_df.to_csv(os.path.join(self.nsw_data_loader.output_dir, "sarimax_results", f"{self.config.task_id}_test_results.csv"), index=False)
    ...
    def train_model(self):
        ...
        fitting_progress_log_df = pd.DataFrame(progress_log)
        os.makedirs(os.path.join(self.nsw_data_loader.output_dir, "sarimax_results"), exist_ok=True)
        fitting_progress_log_df.to_csv(os.path.join(self.nsw_data_loader.output_dir, "sarimax_results", f"{self.config.task_id}_fitting_log.csv"), index=False)
        os.makedirs(os.path.join(self.nsw_data_loader.output_dir, "sarimax_models"), exist_ok=True)
        self.best_model.save(os.path.join(self.nsw_data_loader.output_dir, "sarimax_models", f"{self.config.task_id}_model.pkl"))
...
if __name__ == "__main__":
    ...
    # patch_tst_model.test_model(test_mode=1)
